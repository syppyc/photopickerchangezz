...
public class PhotoPreviewActivity extends AppCompatActivity implements PhotoPagerAdapter.PhotoViewClickListener {
    ...
    private void initViews() {
        ...
        preview_back = (ImageView) findViewById(R.id.preview_back);

        delete_iv = (ImageView) findViewById(R.id.delete_iv);
        number_tv = (TextView) findViewById(R.id.number_tv);
        setup_tv = (TextView) findViewById(R.id.setup_tv);
        ...
        preview_back.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (upIndex >= paths.size()) {
                    upIndex = 0;
                    upPath = paths.get(upIndex);
                }
                if (!TextUtils.isEmpty(upPath)) {
                    paths.remove(upIndex);
                    paths.add(0, upPath);
                }
...
//                Toast.makeText(PhotoPreviewActivity.this, "1111", Toast.LENGTH_SHORT).show();
            }
        });
        setup_tv.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                index = mViewPager.getCurrentItem();
                upIndex = mViewPager.getCurrentItem();
                deletedPath = paths.get(index);
                upPath = paths.get(index);
                setup_tv.setText("封面");
                delete_iv.setVisibility(View.INVISIBLE);
//                Toast.makeText(PhotoPreviewActivity.this, "2222", Toast.LENGTH_SHORT).show();
            }
        });
    ...
    public void updateActionBarTitle() {
        ...
        number_tv.setText(getString(R.string.image_index, mViewPager.getCurrentItem() + 1, paths.size()));

        if (mViewPager.getAdapter().getCount() <= upIndex) {
            upIndex = 0;
        }

        if (mViewPager.getCurrentItem() == upIndex) {
            setup_tv.setText("封面");
            delete_iv.setVisibility(View.INVISIBLE);
        } else {
            setup_tv.setText("设为封面");
